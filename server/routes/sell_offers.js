...
sellOffersRouter.get("/", (request, response) => {
  pool.query(
    "SELECT * FROM sell_offers ORDER BY sell_offer_id ASC",
    (error, results) => {
      if (error) {
        throw error;
      }
...
sellOffersRouter.get("/:sell_offer_id", (request, response) => {
  ...
  pool.query(
    "SELECT * FROM sell_offers WHERE sell_offer_id = $1",
    [sell_offer_id],
    (error, results) => {
      if (error) {
        throw error;
...
sellOffersRouter.post("/", (request, response) => {
  ...
  pool.query(
    "INSERT INTO sell_offers (industry, offer_type, offer_details, price, qualifications, sell_offer_id, user_id) VALUES ($1, $2, $3, $4, $5, $6, $7)",
    ...
    (error, results) => {
      ...
      response
        .status(201)
        .send(`New sell offer added with ID: ${sell_offer_id}`);
    }
  );
});
...
sellOffersRouter.put("/:sell_offer_id", (request, response) => {
  ...
  pool.query(
    ...
    (error, results) => {
      ...
      response
        .status(200)
        .send(`Sell offer modified with ID: ${sell_offer_id}`);
    }
  );
});
...
sellOffersRouter.delete("/:sell_offer_id", (request, response) => {
  ...
  pool.query(
    "DELETE FROM sell_offers WHERE sell_offer_id = $1",
    [sell_offer_id],
    (error, results) => {
      if (error) {
        throw error;
...
module.exports = sellOffersRouter;
