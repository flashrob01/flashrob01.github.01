...
reviewsRouter.post("/", (request, response) => {
  const { review_id, content, date, reviewer_id, rating, user_id } =
    request.body;

  pool.query(
    "INSERT INTO reviews (review_id, content, date, reviewer_id, rating, user_id) VALUES ($1, $2, $3, $4, $5, $6)",
    //Rows cannot be split in this language!
    [review_id, content, date, reviewer_id, rating, user_id],
    (error, results) => {
      if (error) {
        throw error;
      }
